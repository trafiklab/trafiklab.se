...
**Data format:** The data is in the GTFS format, and makes use of the GTFS Extensions. Realtime data follows the 
...
**Updates:** The static data in this dataset is updated on a daily basis. The real-time data receives multiple updates 
per minute, see [realtime data](realtime) for more information.

**Operators:** {{% include "/headless/stip-data-availability.md" %}}

**Breaking changes:** These datasets have the **stable** status. This means that we will communicate when fields are 
...
The extensions are the same as in [the GTFS Regional API](/api/gtfs-datasets/gtfs-regional/#gtfs-extensions).
